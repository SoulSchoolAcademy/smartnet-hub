...
export default function Page() {
  // Tabs
  const tabs = [
    "overview","soulschool","academy","mail","affiliates","crm",
    "scheduler","contacts","world","feeds","wallet","analytics"
  ];
  ...
  // Roster
  const rosterData: Record<string,string[]> = {
  ...
  const [topic, setTopic] = useState<keyof typeof rosterData>("general");
...
@media (max-width:720px){ .topbar{grid-template-columns:1fr auto} .brand h1{display:none} }
      ...
      <div className="tabs" id="tabs">
        {tabs.map((k)=>(
          <div key={k} className={"tab"+(activeTab===k?" active":"")} data-feed={k} onClick={()=>setActiveTab(k)}>
            {k[0].toUpperCase()+k.slice(1)}
          </div>
        ))}
        <div className="tab" id="addTab">＋</div>
      </div>
            ...
            <div className="sb">
              {[
                ...
                <a key={idx} className={"sbbtn"+(active?" active":"")} href="#">
                  ...
                  <div className="sblabel">{label as string}</div>
                  {(unread as number)>0 && <span className="sb-badge">{unread as number}</span>}
